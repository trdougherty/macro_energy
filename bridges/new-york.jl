...
begin
# trying to deal with all the energy files
e2010 = CSV.read(joinpath(city, "energy-2010.csv"), DataFrame)
e2011 = CSV.read(joinpath(city, "energy-2011.csv"), DataFrame)
e2012 = CSV.read(joinpath(city, "energy-2012.csv"), DataFrame)
e2013 = CSV.read(joinpath(city, "energy-2013.csv"), DataFrame)
e2014 = CSV.read(joinpath(city, "energy-2014.csv"), DataFrame)
e2015 = CSV.read(joinpath(city, "energy-2015.csv"), DataFrame)
e2016 = CSV.read(joinpath(city, "energy-2016.csv"), DataFrame)
e2017 = CSV.read(joinpath(city, "energy-2017.csv"), DataFrame)
e2018 = CSV.read(joinpath(city, "energy-2018.csv"), DataFrame)
e2019 = CSV.read(joinpath(city, "energy-2019.csv"), DataFrame)
e2020 = CSV.read(joinpath(city, "energy-2020.csv"), DataFrame)
e2021 = CSV.read(joinpath(city, "energy-2021.csv"), DataFrame)
end;

# ╔═╡ f003afb4-a794-49fa-b5f0-f5078f8bd425
names(e2010)

# ╔═╡ ffb7554a-3490-48a0-b72d-efe81ef5c59b
begin
e2010′ = unique(select(rename(
	e2010, 
	"BBL" => "bbl",
	"BIN" => "bin",
	"Address 1" => "address",
	"City" => "city",
	"Total Floor Space (Sq. Ft.)" => "area_ft",
	"Current Total Site Energy Use (kBtu)" => "energy_kbtu"
), ["bbl","bin","address","city","energy_kbtu","area_ft"]))
e2010′.year = repeat([2010], nrow(e2010′))
end;

# ╔═╡ a95bc84f-683b-4770-af5e-9c86240c07ab
begin
e2011′ = unique(select(rename(
	e2011, 
	"BBL10" => "bbl",
	"BIN (Unique Building Identifier)" => "bin",
	"Address 1" => "address",
	"City" => "city",
	"Total Floor Space (Sq Ft)" => "area_ft",
	"Current Total Site Energy Use (kBtu)" => "energy_kbtu"
), ["bbl","bin","address","city","energy_kbtu","area_ft"]))
e2011′.year = repeat([2011], nrow(e2011′))
end;

# ╔═╡ bdf41c07-a5d7-46d2-861c-49738d6a9566
begin
e2012′ = unique(select(rename(
	e2012, 
	"BBL" => "bbl",
	"BIN" => "bin",
	"Address 1" => "address",
	"City" => "city",
	"Property Floor Area (Buildings and Parking) (ft²)" => "area_ft",
...
e2013′ = unique(select(rename(
	...
	"Address 1" => "address",
	"City" => "city",
	"Property Floor Area (Building(s)) (ft²)" => "area_ft",
	"Site Energy Use (kBtu)" => "energy_kbtu"
), ["bbl","bin","address","city","energy_kbtu","area_ft"]))
e2013′.year = repeat([2013], nrow(e2013′))
end;
...
e2014′.year = repeat([2014], nrow(e2014′))
end;
...
e2015′ = unique(select(rename(
	...
	"Address 1" => "address",
	"City" => "city",
	"Largest Property Use Type - Gross Floor Area (ft²)" => "area_ft",
	"Site Energy Use (kBtu)" => "energy_kbtu"
), ["bbl","bin","address","city","energy_kbtu","area_ft"]))
e2015′.year = repeat([2015], nrow(e2015′))
end;

# ╔═╡ b1261c5f-952c-4feb-bcfa-94a9dd91218b
begin
e2016′ = unique(select(rename(
	filter(x -> x["Number of Buildings"] == 1, e2016), 
	"NYC Borough, Block and Lot (BBL)" => "bbl",
	"NYC Building Identification Number (BIN)" => "bin",
	"Address 1" => "address",
	"City" => "city",
...
e2016′.year = repeat([2016], nrow(e2016′))
end;

# ╔═╡ 0b28c3fe-6c48-4e77-8868-a1c03060c5c6
begin
e2017′ = unique(select(rename(
	filter(x -> x["Number of Buildings"] == 1, e2017), 
	"NYC Borough, Block and Lot (BBL)" => "bbl",
	"NYC Building Identification Number (BIN)" => "bin",
	"Address 1" => "address",
	"City" => "city",
...
e2017′.year = repeat([2017], nrow(e2017′))
end;

# ╔═╡ 43afded1-c942-455b-a67d-ff0442884c2d
begin
...
# ╔═╡ d87edbaa-0675-4046-951d-53011a66fa7d
begin
...
# ╔═╡ 93f4e4ce-3408-4fa8-a8a9-6db5dc3c3884
length(e2021′.bin[1])
...
begin
energyᵣ₁ = filter( x -> 
	typeof(x.energy_kbtu) == String15 && 
	x.energy_kbtu != "Not Available" &&
	x.area_ft != "Not Available", energyᵣ );
	
energyᵣ₁.energy_kbtu = parse.(Float64, energyᵣ₁.energy_kbtu)
energyᵣ₁.area_ft = parse.(Float64, energyᵣ₁.area_ft)
end;
...
footprints_o = unique(select(footprints, [:bin,:geometry]))
@info "Sample of dataframe:" footprints_o
...
# 	energy[:, "id"] 	   = hash.(energy[:,"Full.Address"])
...
git-tree-sha1 = "22b310907358aa6f6f328b8e3860c0f7f9811339"
...
git-tree-sha1 = "64a47fe2e04fe57c6b8bd121e565b78b7d424199"
...
git-tree-sha1 = "048fc23bb2392018c735e3d6d43c4dd8e5cb2f23"
